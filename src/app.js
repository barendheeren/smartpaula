...
app.all('/webhook/vitadock', (req, res) => {
    if (req.query.module_id === '1') {
        ...
        console.log(authorization.oauth_token.replace('/\"/g', ''));
        pool.query('SELECT client FROM connect_vitadock WHERE oauth_access_token = $1', [authorization.oauth_token.replace('/\"/g', '')]).then(result => {
            console.log(result)
